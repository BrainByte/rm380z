...
;BSTACK: EQU     0FFFFH           ; STACK
BSTACK: EQU     65000           ; STACK
;OCSW:   EQU     8000H          ;SWITCH FOR OUTPUT
OCSW:   EQU     18500            ;SWITCH FOR OUTPUT ( ROM LOADS AT 0x2000 + length 0x2860 )
...
;TXTEND: EQU     FF00H         ;TEXT SAVE AREA ENDS
TXTEND: EQU     65280          ;TEXT SAVE AREA ENDS
KBDIN:  EQU     2
...
BSTART: LD SP,BSTACK                    ;*** COLD START ***
        LD A,0FFH
        ;LD A,0H
        LD (OCSW),A
        JP BINIT

BRST08: EX (SP),HL                      ;*** TSTC OR RST 08H ***
        CALL BRST28                     ;IGNORE BLANKS AND
        CP (HL)                         ;TEST CHARACTER
        JP TC1                          ;REST OF THIS IS AT TC1
        RET

BRST10: PUSH AF                         ;*** OUTC OR RST 10H ***
        LD A,(OCSW)                     ;PRINT CHARACTER ONLY
        OR A                            ;IF OCSW SWITCH IS ON
        JP BOUTC			            ;REST OF THIS AT OUTC
        RET

BRST18: CALL EXPR2                      ;*** EXPR OR RST 18H ***
        PUSH HL                         ;EVALUATE AN EXPRESSION
        JP EXPR1                        ;REST OF IT AT EXPR1
        DM 'W'
        RET

BRST20: LD A,H                          ;*** COMP OR RST 20H ***
...
BRST30: POP AF                          ;*** FINISH/RST 30H ***
        CALL FIN                        ;CHECK END OF COMMAND
        JP QWHAT                        ;PRINT "WHAT?" IF WRONG
        DM 'G'
        RET

BRST38: CALL BRST28                     ;*** TSTV OR RST 38H ***
...
PRINT:  LD C,06H                        ;C = # OF SPACES
        CALL BRST08                     ;F NULL LIST & ";"
        DB 3BH
        DB PRI2-$-1
        LD A,CR                         ;GIVE CR-LF AND ;CALL CROUT
        EMT OUTC
        JR RUNSML                       ;CONTINUE SAME LINE

PRI2:   CALL BRST08                     ;IF NULL LIST (CR)
        DB CR
        DB PRI0-$-1
        LD A,CR                         ;GIVE CR-LF AND ;CALL CROUT
        EMT OUTC
...
PRI0:   CALL BRST08                     ;ELSE IS IT FORMAT?
        DM '#'
        DB PRI1-$-1
        CALL BRST18                     ;YES, EVALUATE EXPR.
        LD C,L                          ;AND SAVE IT IN C
        JR PRI3                          ;LOOK FOR MORE TO PRINT

PRI1:   CALL QTSTG                      ;OR IS IT A STRING?
        JR PRI8                          ;IF NOT, MUST BE EXPR.

PRI3:   CALL BRST08                     ;IF ",", GO FIND NEXT
        DM ','
        DB PRI6-$-1
        CALL FIN                        ;IN THE LIST.
        JR PRI0                          ;LIST CONTINUES

PRI6:   LD A,CR                         ;GIVE CR-LF AND ;CALL CROUT
        EMT OUTC                        ; LIST ENDS
        CALL BRST30

PRI8:   CALL BRST18                     ;EVALUATE THE EXPR
        PUSH BC
        CALL PRTNUM                     ;PRINT THE VALUE
        POP BC
        JR PRI3                          ;MORE TO PRINT?
...
RETURN: CALL ENDCHK                     ;THERE MUST BE A CR
        ...
        CALL BRST30                     ;AND WE ARE BACK HOME
...
FR7:    ADD HL,BC                       ;EACH LEVEL IS 10 DEEP - DIS = 09
...
FR8:    LD HL,(LOPPT)                   ;JOB DONE, RESTORE DE
        EX DE,HL
        CALL BRST30                     ;AND CONTINUE

NEXT:   CALL BRST38                     ;GET ADDRESS OF VAR.
...
NX0:    PUSH DE                         ;SAVE TEXT POINTER
        ...
        CALL BRST20                     ;ELSE WE CHECK THEM
...
LET:    CALL SETVAL                     ;*** LET ***
        CALL BRST08                     ;SET VALUE TO VAR
        DM ','                          ;---DISASSEMBLE = INC L
        DB LT1-$-1                      ;---DISASSEMBLE = INC BC
        JR LET                          ;ITEM BY ITEM

LT1:    CALL BRST30                     ;UNTIL FINISH
...
EXPR2:  CALL BRST08                     ;NEGATIVE SIGN?
        DM '-'
        DB XP21-$-1
        LD HL,0000H                     ;YES, FAKE '0-'
        JR XP26                         ;TREAT LIKE SUBTRACT

XP21:   CALL BRST08                     ;POSITIVE SIGN? IGNORE
        DM '+'
        DB XP22-$-1

XP22:   CALL EXPR3                      ;1ST <EXPR3>

XP23:   CALL BRST08                     ;ADD?
        DM '+'
        DB XP25-$-1
...
XP40:   CALL BRST38                     ;NO, NOT A FUNCTION
...
SETVAL: CALL BRST38                     ;*** SETVAL ***
        JP C,QWHAT                      ;"WHAT?" NO VARIABLE
        PUSH HL                         ;SAVE ADDRESS OF VAR.
        CALL BRST08                     ;PASS "=" SIGN
        DM '='
        DB SETV1-$-1
        CALL BRST18                     ;EVALUATE EXPR.
...
GL4:    LD A,CR                         ;REDO ENTIRE LINE
        EMT OUTC                        ;GIVE CR-LF AND ;CALL CROUT
...
QT3:    CALL BRST08                     ;IS IT A '?
        DB 27H
        DB QT4-$-1
        LD A,27H                        ;YES, DO THE SAME
        JR QT1                          ;AS IN "

QT4:    CALL BRST08                     ;IS IT BACK-ARROW?
        DB 5FH
        DB QT5-$-1
        LD A,8DH                        ;YES, CR WITHOUT LF
        CALL BRST10                     ;DO IT TWICE TO GIVE
        CALL BRST10                     ;TTY ENOUGH TIME
        POP HL                          ;RETURN ADDRESS
        JR QT2

QT5:    RET                             ;NONE OF ABOVE

PRTNUM: LD B,00H                        ;*** PRTNUM ***
        CALL CHKSGN                     ;CHECK SIGN
        JP P,PN1                        ;NO SIGN
        LD B,'-'                        ;B=SIGN
        DEC C                           ;'-' TAKES SPACE
...
MVDOWN: LD A,B                          ;*** MVDOWN ***
        SUB D                           ;TEST IF DE = BC
        JP NZ,MVD1                      ;NO, GO MOVE
        LD A,C                          ;MAYBE, OTHER BYTE?
        SUB E
        RET Z                           ;YES, RETURN

MVD1:   DEC DE                          ;ELSE MOVE A BYTE
...
BOUTC:  JR NZ,BOUTC2	    ;IT IS ON
        POP AF              ;IT IS OFF
        RET                 ;RESTORE AF AND RETURN

BOUTC2: POP AF              ;RESTORE THE REGISTER
        EMT OUTC
        CP CR
        RET NZ
        LD A,LF
...
CHKIO:  EMT KBDIN			;CHECK IF CHARACTER AVAILABLE
...
CHKIO2: LD A,00H            ;CLEAR A
        OR A                ;ZET THE Z-FLAG
        POP BC              ;RESTORE THE 'BC' PAIR
        RET                 ;RETURN WITH 'Z' SET.

CI1:    CP 03H              ;IS IT CONTROL-C?
        RET NZ              ;NO, RETURN "NZ"
...
TAB5:   DM 'TO'                         ;"TO" IN "FOR"
        DWA FR1
        DWA QWHAT

TAB6:   DM 'STEP'                       ;"STEP" IN "FOR"
        DWA FR2
        DWA FR3

TAB8:   DM '>='                         ;RELATION OPERATORS
        DWA XP11
        DM '#'
        DWA XP12
        DM '>'
...
EXEC:                                   ;*** EXEC ***
EX0:    CALL BRST28                     ;IGNORE LEADING BLANKS
...
MSG1:   DEFM    'Z80 TINY BASIC 2.0g'   ;BOOT MESSAGE
        DEFB    -1
MSG2:   DEFM    'PORTED BY BRAINBYTE, 2021'
        DEFB    -1
BASV:   DEFM    'TB ROM LOADED'          ;VERSION NUMBER
        DEFB    -1
