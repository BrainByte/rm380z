...
void trace( uint16_t pc, uint8_t op, z80_t* cpu );
...
void trace( uint16_t pc, uint8_t op, z80_t* cpu )
{
    if ( cpu->trace )
    {
        uint8_t data = (uint8_t)( ( cpu->pins & 0xFF0000ULL ) >> 16 );
        char* datax = new char[32];
        toHex( datax, data, "%0.2X" );

        uint16_t af = z80_af( cpu );
        char* afx = new char[32];
        toHex( afx, af, "%0.4X" );

        uint16_t hl = z80_hl( cpu );
        char* hlx = new char[32];
        toHex( hlx, hl, "%0.4X" );

        uint16_t bc = z80_bc( cpu );
        char* bcx = new char[32];
        toHex( bcx, bc, "%0.4X" );

        uint16_t de = z80_de( cpu );
        char* dex = new char[32];
        toHex( dex, de, "%0.4X" );

        char* pcx = new char[32];
        toHex( pcx, pc - 1, "%0.4X" );

        char* opx = new char[32];
        toHex( opx, op, "%0.2X" );

        if ( pc < 856 || pc > 866 )
            std::cout << "PC : " << pcx << " - OP : " << opx << " - HL : " << hlx << " - BC : " << bcx << " - DE : " << dex << " - AF : " << afx << " - DATA : " << datax << std::endl;
